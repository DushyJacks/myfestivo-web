  ...
  return (
    <>
      <motion.div variants={pageItem} className="mb-8">
        ...
        <h1 className="text-3xl font-light tracking-tight text-[var(--color-text)]">Browse events.</h1>
      </motion.div>

      {/* Search & Filters */}
      <motion.div variants={pageItem} className="pt-2 pb-4 mb-6 border-b border-[var(--color-border)]">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-3" suppressHydrationWarning>
          <div className="relative max-w-md w-full">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="pl-9 bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text)] placeholder:text-[var(--color-text-faint)] h-10 rounded-full" />
          </div>
          {/* Sort — moved left of the Filters button */}
          <div className="flex items-center gap-1.5">
            <ArrowUpDown className="w-3 h-3 text-[var(--color-text-faint)]" aria-hidden="true" />
            ...
            <select id="sort-select" value={sortBy} onChange={e => setSortBy(e.target.value as SortOption)}
              ...
              className="bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)] text-xs rounded-full px-3 py-1.5 cursor-pointer">
          ...
          <button onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-xs border transition-colors ${
              showFilters
                ? 'border-[var(--color-accent)] text-[var(--color-accent)] bg-[var(--color-accent-low)]'
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setCategoryFilter(cat)}
              className={`px-4 py-1.5 rounded-full text-xs transition-all duration-200 border ${
                categoryFilter === cat
                  ? "border-[var(--color-accent)] text-[var(--color-accent)] bg-[var(--color-accent-low)] font-medium"
                  : "border-[var(--color-border)] text-[var(--color-text-faint)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]"
              }`}>{cat}</button>
          ))}
        </div>
        ...
        {showFilters && (
          <div className="flex gap-3 mt-3 pt-3 border-t border-[var(--color-border)]">
            <div>
              <span className="text-[9px] font-mono text-[var(--color-text-faint)] mb-1 block tracking-widest">PRICE</span>
            ...
            <div>
              <span className="text-[9px] font-mono text-[var(--color-text-faint)] mb-1 block tracking-widest">TYPE</span>
              <div className="flex gap-1">
      ...
      <motion.div variants={pageItem} className="mb-4 flex items-center justify-between">
        <p className="text-xs font-mono text-[var(--color-text-muted)]">{filtered.length} event{filtered.length !== 1 ? 's' : ''} found</p>
        {(search || categoryFilter !== "All" || priceFilter !== "all" || typeFilter !== "all") && (
          <button onClick={() => { setSearch(""); setCategoryFilter("All"); setPriceFilter("all"); setTypeFilter("all"); setSortBy("date") }}
            className="text-[10px] text-[var(--color-text-faint)] hover:text-[var(--color-text-muted)] transition-colors">Clear all filters</button>
        )}
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full py-32 flex flex-col items-center gap-4">
            <span className="font-mono tracking-widest text-[var(--color-text-faint)] text-xl">— NO EVENTS FOUND —</span>
            ...
            <motion.div key={evt.id} variants={pageItem}>
              <Link href={`/events/${evt.id}`}>
                <GlassCard className={`p-0 hover:scale-[1.01] transition-transform duration-150 ease-out cursor-pointer relative overflow-hidden group h-full flex flex-col ${evt.isInter ? '' : 'border-l-yellow-500/50 border-l-[3px]'}`}>
                  ...
                  <div className="w-full aspect-[16/9] overflow-hidden border-b border-[var(--color-border)] bg-gradient-to-br from-[var(--color-surface-2)] to-[var(--color-surface)] flex items-center justify-center relative">
                    {evt.poster_base64 ? (
                      ...
                      <div className="flex flex-col items-center justify-center gap-2">
                        <div className="w-10 h-10 rounded-xl border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-faint)] text-lg bg-[var(--color-accent-low)]">🎉</div>
                        <span className="text-[10px] text-[var(--color-text-faint)]">No poster</span>
                      </div>
                    )}
                    {/* Pending review overlay — visible only to organizer */}
                    {evt.status === "pending_review" && (
                      <div className="absolute inset-0 bg-[var(--color-bg)]/60 flex flex-col items-center justify-center gap-2 backdrop-blur-[2px]">
                        ...
                        <span className="text-[9px] text-[var(--color-text-faint)] px-4 text-center">Awaiting admin approval — not visible to others</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      <Badge variant="outline" className="border-[var(--color-border)] text-[var(--color-text-muted)] font-normal uppercase tracking-widest text-[10px]">
                        {evt.category}
                      </Badge>
                      <div className="flex gap-2">
                        ...
                        {evt.isInter && (
                          <span className="font-mono text-[10px] px-2 py-0.5 border border-[var(--color-text)] text-[var(--color-text)]">INTER</span>
                        )}
                      </div>
                    </div>

                  <h3 className="font-semibold text-xl mb-1 flex-1 text-[var(--color-text)] group-hover:text-[var(--color-accent)] transition-colors">{evt.title}</h3>
                  <p className="font-light text-[var(--color-text-muted)] text-sm mb-2">{evt.organizer}</p>
                  <p className="text-xs text-[var(--color-text-muted)] mb-4 line-clamp-2">{evt.description}</p>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 font-mono text-[11px] text-[var(--color-text-muted)]">
                            <span className="flex items-center gap-1">
                              ...
                              {evt.hasTime && evt.time && <span className="text-[var(--color-text-faint)] ml-1">at {formatTimeDisplay(evt.time)}</span>}
                            </span>
                            <span className="flex items-center gap-1 text-[var(--color-accent)]"><Users className="w-3 h-3" />{actualRegisteredCount} registered</span>
                          </div>
                          <span className={`font-mono text-[11px] font-medium ${ evt.price > 0 ? 'text-[var(--color-accent)]' : 'text-[var(--color-success)]'}`}>
                            {evt.price > 0 ? `₹${evt.price}` : "FREE"}
                          </span>
                        </div>

                        {/* Capacity bar */}
                        <div className="mt-3 h-1 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
                          <div className={`h-full rounded-full transition-all ${actualRegisteredCount > evt.seats * 0.8 ? 'bg-red-500/60' : actualRegisteredCount > evt.seats * 0.5 ? 'bg-yellow-500/40' : 'bg-[var(--color-accent)]/30'}`}
                  ...
                  {evt.subEvents.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-[var(--color-border)] flex gap-2 flex-wrap">
                      {evt.subEvents.map(se => (
                        <span key={se.id} className="text-[10px] bg-[var(--color-surface-2)] border border-[var(--color-border)] px-2 py-0.5 rounded text-[var(--color-text-muted)]">{se.name}</span>
                      ))}
                    </div>
                  )}
                  </div>
