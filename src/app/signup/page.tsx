      ...
      <motion.div variants={pageItem} className="w-full max-w-lg">
        ...
        <GlassCard className="p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="text-[11px] font-mono tracking-widest uppercase text-white/40 mb-2 block">Full Name</label>
              <Input value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Your full name" className="bg-white/[0.03] border-white/[0.08] text-white placeholder:text-white/30 h-11" required />
            </div>
            ...
            {form.role === "student" && (
              <>
                <div>
                  <label className="text-[11px] font-mono tracking-widest uppercase text-white/40 mb-2 block">College / University</label>
                  <Input value={form.college} onChange={(e) => update("college", e.target.value)} placeholder="e.g. SRM Institute of Science and Technology" className="bg-white/[0.03] border-white/[0.08] text-white placeholder:text-white/30 h-11" required />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[11px] font-mono tracking-widest uppercase text-white/40 mb-2 block">Roll Number</label>
                    <Input value={form.rollNo} onChange={(e) => update("rollNo", e.target.value)} placeholder="RA2211003010001" className="bg-white/[0.03] border-white/[0.08] text-white placeholder:text-white/30 h-11" required />
                  </div>
                  <div>
                    <label className="text-[11px] font-mono tracking-widest uppercase text-white/40 mb-2 block">Department</label>
                    <Input value={form.department} onChange={(e) => update("department", e.target.value)} placeholder="Computer Science" className="bg-white/[0.03] border-white/[0.08] text-white placeholder:text-white/30 h-11" required />
                  </div>
                </div>
                <div>
            ...
            <Button type="submit" disabled={loading} className="w-full bg-white text-black hover:bg-white/90 font-medium h-11 transition-colors mt-2">
              {loading ? (
